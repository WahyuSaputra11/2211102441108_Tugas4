...
public class Enemy extends Actor
{
    ...
    public boolean bisa_gerak(int ARAH_GRK)
    {
        ...
        switch (ARAH_GRK)
        {
            case KANAN : Obstacle = getOneObjectAtOffset(1,0,ObjectSpecial.class);
                         if (Obstacle!=null || getX()==9) status=false; else status=true; break;
            
            case KIRI  : Obstacle = getOneObjectAtOffset(-1,0,ObjectSpecial.class);
                         if (Obstacle!=null || getX()==0) status=false; else status=true; break;
            
            case ATAS  : Obstacle = getOneObjectAtOffset(0,-1,ObjectSpecial.class);
                         if (Obstacle!=null || getY()==0) status=false; else status=true; break;
          
            case BAWAH : Obstacle = getOneObjectAtOffset(0,1,ObjectSpecial.class);
    ...
    {
        Actor Obstacle = getOneObjectAtOffset(0,0,ObjectSpecial.class);
        if (Obstacle!=null)
            return true;
        else
            return false;
